...
export function initDesks(state, catalog) {
  ...
  if (state.save_schema == null || state.save_schema < 14) state.save_schema = 14;
...
export function applyDeskMonthlyPosture(state, scale = 1) {
  ...
  const constCourt = state.desks.const_court;
  if (constCourt) {
    if (constCourt.posture === "docket_open") {
      f.legitimacy = clamp(f.legitimacy + 0.3 * s);
      f.assembly_function = clamp(f.assembly_function + 0.2 * s);
      f.war_risk = clamp(f.war_risk - 0.1 * s);
    } else if (constCourt.posture === "bind_justice") {
      f.legitimacy = clamp(f.legitimacy + 0.35 * s);
      f.presidency_cohesion = clamp(f.presidency_cohesion + 0.25 * s);
      f.assembly_function = clamp(f.assembly_function + 0.15 * s);
      f.war_risk = clamp(f.war_risk - 0.15 * s);
    } else if (constCourt.posture === "stall") {
      f.legitimacy = clamp(f.legitimacy - 0.3 * s);
      f.assembly_function = clamp(f.assembly_function - 0.2 * s);
      constCourt.capacity = clamp(constCourt.capacity - 0.15 * s);
    } else if (constCourt.posture === "yield_republics") {
      f.legitimacy = clamp(f.legitimacy - 0.4 * s);
      f.presidency_cohesion = clamp(f.presidency_cohesion - 0.2 * s);
      if (state.units.SI) state.units.SI.federal_trust = clamp(state.units.SI.federal_trust - 0.2 * s);
      if (state.units.HR) state.units.HR.federal_trust = clamp(state.units.HR.federal_trust - 0.2 * s);
      if (state.flags.confederal_talks_open) {
        f.war_risk = clamp(f.war_risk + 0.25 * s);
      }
    }
    if (state.flags.const_court_bind_justice || state.flags.justice_court_bind) {
      f.legitimacy = clamp(f.legitimacy + 0.1 * s);
    }
    if (state.flags.ssup_docket_watch || state.flags.ssup_court_liaison) {
      f.war_risk = clamp(f.war_risk - 0.05 * s);
    }
  }

  const provinces = state.desks.provinces;
  if (provinces) {
    if (provinces.posture === "vote_independent" || state.flags.provinces_vote_independently) {
      f.legitimacy = clamp(f.legitimacy + 0.2 * s);
      f.presidency_cohesion = clamp(f.presidency_cohesion - 0.1 * s);
      f.war_risk = clamp(f.war_risk - 0.1 * s);
      if (state.units.VO) state.units.VO.federal_trust = clamp(state.units.VO.federal_trust + 0.15 * s);
      if (state.units.XK) state.units.XK.federal_trust = clamp(state.units.XK.federal_trust + 0.15 * s);
      if (state.units.RS) state.units.RS.federal_trust = clamp(state.units.RS.federal_trust - 0.1 * s);
    } else if (provinces.posture === "mediate_seats" || state.flags.provinces_mediate) {
      f.presidency_cohesion = clamp(f.presidency_cohesion + 0.35 * s);
      f.legitimacy = clamp(f.legitimacy + 0.15 * s);
      f.war_risk = clamp(f.war_risk - 0.1 * s);
    } else if (provinces.posture === "observe" || state.flags.provinces_observe) {
      f.legitimacy = clamp(f.legitimacy + 0.1 * s);
      provinces.capacity = clamp(provinces.capacity + 0.1 * s);
    } else if (provinces.posture === "bloc_aligned" || state.flags.provinces_bloc_tight) {
      f.presidency_cohesion = clamp(f.presidency_cohesion + 0.15 * s);
      if (state.units.VO) state.units.VO.federal_trust = clamp(state.units.VO.federal_trust - 0.1 * s);
      if (state.units.XK) state.units.XK.federal_trust = clamp(state.units.XK.federal_trust - 0.1 * s);
      if (state.flags.confederal_talks_open) {
        f.war_risk = clamp(f.war_risk + 0.2 * s);
        f.presidency_cohesion = clamp(f.presidency_cohesion - 0.15 * s);
      }
    }
    if (state.flags.ssup_province_dossier) {
      f.war_risk = clamp(f.war_risk - 0.05 * s);
    }
    if (state.flags.presidency_province_quorum) {
      f.presidency_cohesion = clamp(f.presidency_cohesion + 0.15 * s);
    }
  }
  ...
  const labor = state.desks.labor;
  if (labor) {
    if (labor.posture === "social_peace" || state.flags.labor_social_peace) {
      f.labor_peace = clamp((f.labor_peace ?? 50) + 0.25 * s);
      f.legitimacy = clamp(f.legitimacy + 0.1 * s);
      labor.loyalty = clamp(labor.loyalty + 0.1 * s);
    } else if (labor.posture === "strike_cool" || state.flags.labor_strike_cool) {
      f.labor_peace = clamp((f.labor_peace ?? 50) + 0.2 * s);
      f.war_risk = clamp(f.war_risk - 0.1 * s);
    } else if (labor.posture === "reform_support" || state.flags.labor_reform_support) {
      f.labor_peace = clamp((f.labor_peace ?? 50) + 0.15 * s);
      f.reform_momentum = clamp(f.reform_momentum + 0.15 * s);
      labor.capacity = clamp(labor.capacity + 0.1 * s);
    } else if (labor.posture === "wage_corridor" || state.flags.labor_wage_corridor) {
      f.labor_peace = clamp((f.labor_peace ?? 50) + 0.1 * s);
      f.inflation = clamp(f.inflation - 0.1 * s);
    } else if (labor.posture === "harden_line" || state.flags.labor_harden_line) {
      f.labor_peace = clamp((f.labor_peace ?? 50) - 0.1 * s);
      f.war_risk = clamp(f.war_risk + 0.15 * s);
      labor.agenda_tension = clamp(labor.agenda_tension + 0.15 * s);
    }
  }

  const agriculture = state.desks.agriculture;
  if (agriculture) {
    if (agriculture.posture === "food_security" || state.flags.agriculture_food_security) {
      f.food_security = clamp((f.food_security ?? 50) + 0.25 * s);
      f.legitimacy = clamp(f.legitimacy + 0.1 * s);
      agriculture.loyalty = clamp(agriculture.loyalty + 0.1 * s);
    } else if (agriculture.posture === "procurement_soft" || state.flags.agriculture_procurement_soft) {
      f.food_security = clamp((f.food_security ?? 50) + 0.2 * s);
      f.inter_republic_trade = clamp(f.inter_republic_trade + 0.1 * s);
    } else if (agriculture.posture === "farm_relief" || state.flags.agriculture_farm_relief) {
      f.food_security = clamp((f.food_security ?? 50) + 0.15 * s);
      f.war_risk = clamp(f.war_risk - 0.1 * s);
    } else if (agriculture.posture === "reform_support" || state.flags.agriculture_reform_support) {
      f.food_security = clamp((f.food_security ?? 50) + 0.15 * s);
      f.reform_momentum = clamp(f.reform_momentum + 0.15 * s);
      agriculture.capacity = clamp(agriculture.capacity + 0.1 * s);
    } else if (agriculture.posture === "harden_quota" || state.flags.agriculture_harden_quota) {
      f.food_security = clamp((f.food_security ?? 50) - 0.1 * s);
      f.war_risk = clamp(f.war_risk + 0.15 * s);
      agriculture.agenda_tension = clamp(agriculture.agenda_tension + 0.15 * s);
    }
    if (state.flags.agriculture_siv_bind) {
      f.siv_authority = clamp(f.siv_authority + 0.1 * s);
      f.food_security = clamp((f.food_security ?? 50) + 0.1 * s);
    }
  }
...
export function syncDeskFlagsFromPosture(state) {
  ...
  const ssipD = state.desks.ssip;
  if (ssipD) {
    state.flags.ssip_ec_channel = ssipD.posture === "ec_dialogue";
    state.flags.ssip_nonaligned_hold = ssipD.posture === "nonaligned_hold";
    state.flags.ssip_bilateral_quiet = ssipD.posture === "bilateral_quiet";
    state.flags.ssip_paralyzed = ssipD.posture === "paralyzed";
  }
  const courtD = state.desks.const_court;
  if (courtD) {
    state.flags.const_court_docket_open = courtD.posture === "docket_open";
    state.flags.const_court_stalled = courtD.posture === "stall";
    state.flags.const_court_yields = courtD.posture === "yield_republics";
    state.flags.const_court_bind_justice = courtD.posture === "bind_justice";
  }
  const provD = state.desks.provinces;
  if (provD) {
    state.flags.provinces_bloc_tight = provD.posture === "bloc_aligned";
    state.flags.provinces_observe = provD.posture === "observe";
    state.flags.provinces_mediate = provD.posture === "mediate_seats";
  ...
  const laborD = state.desks.labor;
  if (laborD) {
    state.flags.labor_social_peace = laborD.posture === "social_peace" || state.flags.labor_social_peace;
    state.flags.labor_strike_cool = laborD.posture === "strike_cool" || state.flags.labor_strike_cool;
    state.flags.labor_reform_support = laborD.posture === "reform_support" || state.flags.labor_reform_support;
    state.flags.labor_wage_corridor = laborD.posture === "wage_corridor" || state.flags.labor_wage_corridor;
    state.flags.labor_harden_line = laborD.posture === "harden_line";
  }
  const agriD = state.desks.agriculture;
  if (agriD) {
    state.flags.agriculture_food_security = agriD.posture === "food_security" || state.flags.agriculture_food_security;
    state.flags.agriculture_procurement_soft = agriD.posture === "procurement_soft" || state.flags.agriculture_procurement_soft;
    state.flags.agriculture_farm_relief = agriD.posture === "farm_relief" || state.flags.agriculture_farm_relief;
