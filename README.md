...
- Title / new game / continue (`localStorage` key `sfrj1990.v1`, schema ≥ 4) / encyclopedia (timeline + E1–E5 + desks/dialogue + attention + E3 path)
...
- **Šalteri** (department desks): JNA, SIV, Predsjedništvo, SSUP (SDB leash), SSP, Financije, TO, NBJ, **Pravosuđe** — loyalty / capacity / agenda + actions. Posture chips (overflow-safe) on the agency strip; posture feeds monthly drift, federal control, election weights, Presidency votes, and endings
- **Razgovori**: multi-visit branching chats — Drnovšek revisits through 15 May 1990; Kučan/Jović/Marković/Mesić/Tuđman/Kadijević when flags/time unlock nodes. Flags + desk stats, not an LLM
...
- Acts I–V with historically grounded cards plus desk/dialogue-gated cards (SSP/justice Act I, conflict warn + FER May, …). Vertical Slice A ends 15 May 1991 and locks endings E1–E5
...
data/desks.json     department desk stats + actions (9 desks)
data/dialogue/      branching institutional chats (multi-visit entries)
data/i18n/hr.json   Croatian UI chrome
data/i18n/en.json   English chrome (expanded Pass 4)
...
**Smoke:** `node scripts/smoke.mjs` boots state and asserts desks / dialogue / attention / Pass 4 event ids (no browser).
